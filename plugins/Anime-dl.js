...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    let isDBS = command.toLowerCase().includes('lat') || command.toLowerCase().includes('dbs')
    let providerName = isDBS ? 'AnimeDBS' : 'TioAnime'
    let baseUrl = isDBS 
...
handler.command = /^(descargaranimeSub|descargaranimeLat|descargaranimeDBS)$/i
handler.tags = ['anime']
